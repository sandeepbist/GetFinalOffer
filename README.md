...
### 2. Adaptive Recall Expansion (Dynamic Thresholding)
One of the hardest challenges in Vector Search is choosing the similarity threshold.
* *Too Strict (>0.5):* You get zero results for niche queries.
* *Too Loose (<0.10):* You get hallucinations and irrelevant candidates.

**The Solution:** I implemented **Two-Stage Dynamic Thresholding** within the SQL transaction:
1.  **Phase 1 (Precision Mode):** The engine executes a strict search (0.32 threshold) to find high-confidence matches.
2.  **Phase 2 (Recall Mode):** If the result set size is `< 5`, the engine *automatically* re-runs the query with a wider net (0.10 threshold) and flags the results as "Broad Matches."
* *Result:* The recruiter never sees a blank screen, while the UI transparently communicates the confidence level of each result.
