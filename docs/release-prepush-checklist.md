...
- [ ] Curated graph sources are tracked:
  ...
  - [ ] `data/skill-graph/taxonomy.v3.json`
...
The committed taxonomy is `data/skill-graph/taxonomy.v3.json` (generated by `pnpm graph:generate-v3`); the v1 file is the curated seed.

- [ ] Validate committed taxonomies:
```bash
pnpm graph:validate -- data/skill-graph/taxonomy.v1.json
pnpm graph:validate -- data/skill-graph/taxonomy.v3.json
```
- [ ] Sync taxonomy (v3 is the production source):
```bash
pnpm graph:sync-taxonomy -- --input data/skill-graph/taxonomy.v3.json
```
- [ ] Backfill IDF:
```bash
...
pnpm graph:warmup
```
- [ ] Ensure the hybrid search RPC exists in the target database:
```bash
psql "$DATABASE_URL" -f db/sql/match_candidates_hybrid.sql
```
